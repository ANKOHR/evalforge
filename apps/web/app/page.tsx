...
export default function Home() {
  ...
  return <main>
    <nav className="topbar shell"><a className="brand" href="#overview"><span className="brand-mark">E</span>EvalForge</a><div className="nav-links"><a href="#comparison">Comparisons</a><a href="#cases">Test cases</a><a href="#method">Methodology</a><span className="nav-status">PROVIDER-FREE DEMO</span></div></nav>
    <section className="hero shell" id="overview"><div className="eyebrow"><span className="pulse"/> HELD-OUT EVALUATION <span className="slash">/</span> RELEASE GATE</div><div className="hero-grid"><div><h1>Quality gates for <em>AI systems.</em></h1><p className="hero-copy">EvalForge makes RAG and agent regressions visible before they reach production: frozen cases, deterministic graders, failure groups and a release decision you can reproduce.</p><div className="hero-actions"><a className="button primary" href="#comparison">Inspect comparison <span>↓</span></a><a className="button quiet" href="#method">Read methodology ↓</a></div></div><div className="release-card"><p className="kicker orange-text">RELEASE DECISION</p><p className="release-title">BLOCK RELEASE</p><p>Candidate v2 improves fact correctness, but citation correctness falls below the configured gate.</p><div className="release-footer"><span>120 total cases</span><span>24 held out</span></div></div></div></section>
    ...
    <footer className="footer shell"><div><span className="brand-mark small">E</span><span>EvalForge</span></div><span>Regression testing for LLM, RAG and agent systems.</span><span>Source publication pending</span></footer>
  </main>;
}
